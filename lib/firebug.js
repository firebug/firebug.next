...
Locale.registerStringBundle("chrome://firebug/locale/actor-inspector.properties");
...
require("./debug/transport-hooks.js");

// Should only be available in 'development' mode.
require("./debug/actor-inspector/inspector-panel.js");
...
/** @lends Firebug */
{
  ...
  registerOverlay: function(overlay) {
    ...
    let onApplyOverlay = (eventId, toolbox, panelFrame) => {
      ...
      // xxxHonza: this can be removed when:
      // https://bugzilla.mozilla.org/show_bug.cgi?id=1126274
      // is fixed (this is just to make sure onReady is executed
      // soon enough even if 'toolbox-create' isn't yet fired).
  ...
  shutdown: function(reason) {
    ...
    gDevTools.off("toolbox-create", this.onToolboxCreated);
  ...
  onToolboxCreated: function(eventId, toolbox) {
    FBTrace.sysout("firebug.onToolboxCreated;", toolbox);
  ...
  onToolboxReady: function(event, toolbox) {
    ...
    // xxxHonza: this can be removed when:
    // https://bugzilla.mozilla.org/show_bug.cgi?id=1126274
    // This is to make sure that chrome.onReady is executed
    // if the default selected panel isn't overlayed and onReady
    // not executed in onApplyOverlay method.
    if (!chrome.overlays) {
      chrome.onReady("", toolbox);
    }
