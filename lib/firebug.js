...
/** @lends Firebug */
{
  ...
  registerOverlay: function(overlay) {
    ...
    let onApplyOverlay = (eventId, toolbox, panelFrame) => {
      ...
      // Create new {@Chrome} instance for the current toolbox if it doesn't
      // exist yet. The panel 'init' event is send before 'toolbox-ready'
      // event and we need the chrome object now (to store overlay instances
      // created below).
      let chrome = this.getChrome(toolbox);
      if (!chrome) {
        chrome = new Chrome(toolbox);
        this.chromes.set(toolbox.target, chrome);
      }
      ...
      try {
        ...
        // it up at the end.
        chrome.overlays[overlay.id] = instance;
  ...
  onToolboxReady: function(event, toolbox) {
    ...
    // Chrome instance should exist already (see the registerOverlay method).
    if (!this.getChrome(toolbox)) {
      TraceError.sysout("firebug.onToolboxReady; ERROR no Chrome " +
        "instance for the current toolbox!", toolbox);
    }

    StartButton.onToolboxReady(toolbox);
  },
  ...
  onToolboxDestroyed: function(eventId, target) {
    ...
    StartButton.onToolboxDestroyed(target);
  ...
  getChrome: function(toolbox) {
    return this.chromes.get(toolbox.target);
  },
