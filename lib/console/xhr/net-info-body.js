...
const NetInfoBody = domplate(Rep,
{
  ...
  hideParams: function(request) {
    return !request.urlParams || !request.urlParams.length;
  },
  ...
  updateHeaders: function(netInfoBody, request, tab) {
    ...
    client.getRequestHeaders(request.actorId, response => {
      ...
      if (!netInfoBody.requestHeadersPresented) {
        ...
        request.requestHeadersText = response.headersSource;
        request.requestHeaders = response.headers;
    ...
    client.getResponseHeaders(request.actorId, response => {
      ...
      if (!netInfoBody.responseHeadersPresented) {
        ...
        request.responseHeadersText = response.headersSource;
  ...
  updateResponse: function(netInfoBody, request, tab) {
    ...
    if (request.discardResponseBody) {
      Dom.clearNode(responseTextBox);
      let tag = SPAN({"class": "netInfoResponseDiscarded"},
        Locale.$STR("xhrSpy.responseBodyDiscarded"))
      tag.append({}, responseTextBox);
      return;
    }

    request.getResponseContent().then(content => {
