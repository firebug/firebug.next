...
/** @lends ConsoleOverlay */
{
  ...
  onReady: function(options) {
    ...
    this.originalSidebarDestroy = jsterm._sidebarDestroy;
    jsterm._sidebarDestroy = this._sidebarDestroy;
  ...
  destroy: function() {
    ...
    jsterm._sidebarDestroy = this.originalSidebarDestroy;
  ...
  /**
   * Monkey patch the _sidebarDestroy so we don't destroy
   * the whole sidebar on console clear or on ESCAPE key pressed
   */
  _sidebarDestroy: function () {
    ...
    if (this.sidebar) {
      this.sidebar.select("commandEditor");
      this.sidebar.removeTab("variablesview")
    }
  }
});
