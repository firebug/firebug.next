...
/** @lends MonitorActor */
{
  ...
  onExamineResponse: function(event) {
    ...
    this.targets.forEach(target => {
      ...
      let targetFrame = getTopFrameElementForTarget(target);
      let requestFrame = getTopFrameElementForRequest(httpChannel);

      // xxxHonza: are top frames supposed to be the same?
      Trace.sysout("monitorActor.onExamineResponse; top frames " +
        (targetFrame == requestFrame), {
        targetFrame: targetFrame,
        requestFrame: requestFrame
      });

      // Only send the message to the logger that lives within
      // the request's parent browser tab.
      // xxxHonza: this doesn't seem to be working. Are these two
      // browsers supposed to be the same?
      if (target == tab.linkedBrowser) {
