...
const { registerActor } = devtools["require"]("devtools/server/actors/common");
...
/** @lends RemoteLogging */
{
  ...
  attachLogger: function(target, form) {
    ...
    // If this error happens the logger actor is not properly registered
    // on the backend (could be in a child process).
    if (!form.firebugLogger) {
      TraceError.sysout("remoteLogging.attachLogger; ERROR no logger", form);
      this.initDeffered.resolve(false);
      return;
    }
