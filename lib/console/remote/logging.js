...
/** @lends RemoteLogging */
{
  ...
  onToolboxReady: function(eventId, toolbox) {
    ...
    let loggerModuleUri = "resource://firebug-next-at-getfirebug-dot-com/lib/console/remote/logger-actor.js";
    ...
    target.client.listTabs(({ actorRegistryActor }) => {
      ...
      registry.registerActor(moduleUri).then(actorFront => {
        FBTrace.sysout("!!!remoteLogging.onToolboxReady; remote actor registered, actor front:", arguments);

        // xxxHonza: global helloWorld actor would be passed in as an argument,
        // while tab helloWorld actor needs to be fetched from the selected tab.
        target.client.listTabs(({ helloActor, tabs, selected }) => {
          FBTrace.sysout("!!!remoteLogging.onToolboxReady; list tabs 2", arguments);

          helloActor = tabs[selected].helloActor;
      ...
      registry.registerActor(loggerModuleUri).then(actorFront => {
        FBTrace.sysout("!!!remoteLogging.onToolboxReady; logger actor registered, actor front:", arguments);

        target.client.listTabs(({ tabs, selected }) => {
          FBTrace.sysout("!!!remoteLogging.onToolboxReady; logger actor list tabs 2", arguments);

          let logger = LoggerFront(target.client, tabs[selected]);
          logger.attach().then(() => {
            FBTrace.sysout("!!!remoteLogging.onToolboxReady; logger attached", arguments);
          });
        });
      });
    });
