...
/** @lends RemoteLogging */
{
  ...
  initialize: function(Firebug) {
    ...
    if (useModuleRegistration()) {
      Trace.sysout("remoteLogging.initialize; use module registration");

      DebuggerServer.registerModule(loggerModuleUri, {
  ...
  shutdown: function(Firebug) {
    ...
    if (DebuggerServer.isModuleRegistered(loggerModuleUri)) {
      DebuggerServer.unregisterModule(loggerModuleUri);
    }
  },
 ...
 *
 * So, use the old (now deprecated) way that registers directly
 * the actor module (doesn't work for remote devices though).
 */
function useModuleRegistration(browserDoc) {
  if (System.isMultiprocessEnabled(browserDoc)) {
    return true;
  }
