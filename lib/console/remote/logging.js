...
let RemoteLogging = {
  ...
  onExamineResponse: function(event) {
    ...
    let {tab, browser} = getTabAndBrowserFromHttpChannel(httpChannel);
    ...
    for (let parsedMessage of parsedMessages) {
      for (let msg of parsedMessage) {
        ...
        //browser.contentWindowAsCPOW.console.log(...msg.logs);
      }
    }
  },
  ...
  logMessage: function(msg, tab) {
    ...
    let packet = messageToPseudoPacket(msg);
    let consoleMessage = new ConsoleMessage(packet);
...
function getTabAndBrowserFromHttpChannel(httpChannel)
{
  ...
  return {tab: tab, browser: topFrame};
}

function messageToPseudoPacket(msg)
{
  return {
    level: msg.type,
    timestamp: new Date(), // TODO fix it
    private: false,
    filename: msg.backtrace.split(":")[0],
    lineNumber: msg.backtrace.split(":")[1],
    arguments: msg.logs,
  };
}
