...
/** @lends LoggerActor */
{
  ...
  initialize: function(conn, parent) {
    ...
    try {
      let moduleUrl = "resource://firebug-at-software-dot-joehewitt-dot-com/" +
        "lib/console/remote/http-monitor.js"

      // Setup HTTP observer in the parent process. It isn't possible to
      // register "http-on-examine-response" observers is child processes.
      DebuggerServer.setupInParent({
        module: moduleUrl,
        setupParent: "initialize",
      });

      let { addMessageListener } = DebuggerServer.parentMessageManager;

      let onHeaders = DevToolsUtils.makeInfallible(function (event) {
        Cu.reportError("Headers received!");

        // TODO: forward to onExamineHeaders.
      });

      // Listen for incoming HTTP headers.
      addMessageListener("http-monitor:examine-headers", onHeaders);
    } catch (err) {
      Cu.reportError(err);
    }
  },
  ...
  attach: method(expectState("detached", function() {
    ...
    try {
      ...
      Trace.sysout("loggerActor.attach; " + err, err);
    }
  ...
  detach: method(expectState("attached", function() {
    ...
    try {
      ...
      Trace.sysout("loggerActor.detach; " + err, err);
    }
  }), {
    request: {},
    response: {
