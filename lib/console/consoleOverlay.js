...
/** @lends ConsoleOverlay */
{
  ...
  onVariablesViewOpen: function(eventId, view, options) {
    ...
    let toolbox = view.toolbox;
    toolbox.target.activeTab.attachThread({}, (response, threadClient) => {
      let provider = new DomProvider(threadClient);
      let tree = new DomTree(provider);

      let input = {
        object: options.objectActor
      };

      // Render UI
      tree.replace(container, input);
    });
  },
