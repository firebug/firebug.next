...
Locale.$STRF = function(name, args) {
  ...
  // _() returns a value same sa the identifier's if it can't find
  ...
  // in a cahr sequence, I think it wouldn't evaluate to true. Yet, Came
...
Locale.$STRP = function(name, index, args) {
  if(index === void(0))
    index = 0;
  // Move the plural form number to the first index of the array, if
  // it's no there.
  if(index != 0)
    [arguments[index], arguments[0]] = [arguments[0], arguments[index]];
