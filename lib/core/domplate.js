...
DomplateTag.prototype =
{
  ...
  compileMarkup: function()
  {
    ...
    fnBlock.push(', s', i);
    fnBlock.push(') {\n');
    //fnBlock.push('Trace.sysout("domplate.renderer; executed", arguments);\n');
    //fnBlock.push('Trace.sysout("domplate.renderer; executed this:", this);\n');
    fnBlock.push('try {\n');
    ...
    fnBlock.push('} catch (err) {;\n');
    fnBlock.push('  Trace.sysout("domplate.renderer; EXCEPTION " + err, err);\n');
    fnBlock.push('}\n');
    ...
    var js = fnBlock.join("");
    //Trace.sysout("domplate.compileMarkup; ", js);
  ...
  compileDOM: function()
  {
    ...
    function __prop__(elem, name, value)
    {
    ...
    {
      TraceError.sysout("domplate.compileDOM; EXCEPTION " + exc, exc);
  ...
  generateDOM: function(path, blocks, args)
  {
    ...
    //Trace.sysout("domplate.generateDOM; Props", this.props);
...
function readPartNames(val, vars)
{
  if (val instanceof Parts)
  {
    ...
    //Trace.sysout("domplate.readPartNames; ", vars.slice(0));
  }
}
...
/*Trace.sysout("domplate.generateArg; ", {
vals: vals,
parts: val.parts
});*/
...
/** @lends Renderer */
{
  ...
  {
    //Trace.sysout("domplate.Renderer.renderHTML; ", arguments);
    ...
    {
      TraceError.sysout("domplate.renderHTML; EXCEPTION " + e, e);
    }
  },
  ...
  insertRows: function(args, after, self)
  {
    ...
    var table = doc.createElementNS(XHTML_NS, "table");

    // Make sure to create <tbody> element. When Domplate is running
    // inside XUL documents the element is not automatically
    // created.
    // See also issue #14 Console Service ERROR "parent is undefined"
    try {
      setHTML(table, "<tbody>" + html + "</tbody>");
    }
    catch (err) {
      TraceError.sysout("domplate.insertRows; EXCEPTION " + err, {
        err: err,
        html: html
      });
    }
    ...
    domArgs.push.apply(domArgs, outputs);
        ...
        /*Trace.sysout("domplate.insertRows; ", {
    domArgs: domArgs,
    tagDomArgs: this.tag.domArgs,
    renderDOM: this.tag.renderDOM.toSource()
    });*/
  ...
  insertNode: function(args, doc, inserter, self)
  {
    ...
    /*Trace.sysout("domplate.insertNode; ", {
    domArgs: domArgs,
    tagDomArgs: this.tag.domArgs,
    renderDOM: this.tag.renderDOM.toSource()
    });*/
  ...
  replace: function(args, parent, self)
  {
    ...
    var root;
    if (parent.nodeType == 1 /*Node.ELEMENT_NODE*/)
    {
      setHTML(parent, html);
    ...
    else
    {
      ...
      var womb = parent.createElementNS(XHTML_NS, "div");
      setHTML(womb, html);
    ...
    {
      TraceError.sysout("domplate.replace; EXCEPTION " + exc, exc);
  ...
  append: function(args, parent, self)
  {
    ...
    var womb = parent.ownerDocument.createElementNS(XHTML_NS, "div");
    setHTML(womb, html);
...
// APIs for associating elements with data. The data is currently stored as
// expandos on the elements, or in the case of elements from unprivileged
// content, expandos on the elements' unwaived xray wrappers. Note that the
// latter expandos are local to the current compartment, and thus, this file.
function getElementData(elem, name) {
  return elem["$domplate_" + name];
}

function setElementData(elem, name, value) {
  elem["$domplate_" + name] = value;
}

// The equivalent of "elem.innerHTML = html", but parsing the string with the
// XML parser rather than as HTML. This makes sure that domplate's idea of the
// DOM matches with what was actually constructed, in the face of e.g. nested
// <a> tags. See #98.
function setHTML(elem, html)
{
  while (elem.firstChild)
    elem.removeChild(elem.firstChild);
  elem.appendChild(parseToHTML(elem.ownerDocument, html));
}
