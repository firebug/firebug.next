...
try {
  ...
  // xxxFlorent: TODO either we make FBTrace an observer of option changes
  // Or we just use firefox command line options to trace certain files.
}
...
function TraceWrapper(tracer, option) {
  function createMethodWrapper(method) {
    return function(...args) {
      ...
      if (tracer[option]) {
        if (method === "sysout" && !traceServiceLoaded) {
          // Invoking console.log() will send the logs to the Browser Console
          // and also dump them to the OS console.
          console.log(`${option} - ${args[0]}`, args.slice(1));
        }
        tracer[method].apply(tracer, args);
      }
    };
  }
