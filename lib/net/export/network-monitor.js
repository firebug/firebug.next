...
/** @lends NetworkMonitor */
{
  ...
  initialize: function(options) {
    ...
    this.toolbox.target.makeRemote().then(() => {
  ...
  startMonitoring: function(aClient, aTabGrip, aCallback) {
    Trace.sysout("NetworkMonitor.startMonitoring;", arguments);
    ...
    if (!aTabGrip) {
      TraceError.sysout("No tab grip found!");
      return;
    }
  ...
  exportCollectedData: function(collector, fileName) {
    ...
    return Exporter.buildHAR(context, jsonp, items).then(jsonString => {
