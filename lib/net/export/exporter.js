...
/** @lends Exporter */
{
  ...
  exportData: function(context, jsonp, items, file) {
    ...
    return this.buildJSON(context, items).then(json => {
  ...
  copyData: function(context, jsonp, items) {
    ...
    this.buildHAR(context, jsonp, items).then(jsonString => {
      if (jsonString) {
  ...
  buildJSON: function(context, items, forceExport) {
    let deferred = defer();
    let { client, form } = context.toolbox.target;

    client.attachConsole(form.consoleActor, [], (aResponse, client) => {
      // Export all data into a JSON string.
      let builder = new HarBuilder();
      builder.build(context, items, client).then(jsonData => {
