...
var Exporter = {
  exportData: function(context, jsonp, items) {
    ...
    if (jsonp) {
      let callbackName = Options.getPref(prefDomain + "jsonpCallback");
    ...
    if (Options.getPref(prefDomain + "showPreview")) {
      let viewerURL = Firebug.getPref(prefDomain + "viewerURL");
      if (viewerURL) {
  ...
  getTargetFile: function(context, jsonp) {
    ...
    if (Options.getPref(prefDomain + "compress")) {
      defaultFileName += ".zip";
    }
  ...
  saveToFile: function(file, jsonString, context, jsonp) {
    ...
    try {
      ...
      TraceError.sysout("netexport.Exporter; Failed to zip log file " + err.toString());
