...
/** @lends HarBuilder */
{
  ...
  buildResponse: function(file) {
    ...
    response.bodySize = file.contentSize;

    if (!response.bodySize && file.responseContent &&
      file.responseContent.content) {
      let content = file.responseContent.content;
      response.bodySize = content.transferredSize || content.size;
    }
