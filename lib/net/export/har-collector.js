...
/** @lends HarCollector */
  ...
  stop: function() {
    ...
    // The monitor can't finish immediately, it needs to wait for
    ...
    this.waitForResponses().then(() => {
      Trace.sysout("HarCollector.stop; DONE Page loaded!");
  ...
  waitForResponses: function() {
    Trace.sysout("HarCollector.waitForResponses; " + promises.length);
