...
/** @lends NetworkOverlay */
{
  ...
  onApplyTheme: function(win, oldTheme) {
    ...
    waitForLoad(win).then(() => this.applyFirebugLayout(win));
  },
  ...
  onUnapplyTheme: function(win, newTheme) {
    ...
    waitForLoad(win).then(() => this.unapplyFirebugLayout(win));
  },
  ...
  applyFirebugLayout: function(win) {
    ...
    // Remove also the original 'clear' button from the UI.
    ...
    this.splitterBox = BOX({"id": "panelSplitterBox"}).build(splitter);
  },
  ...
  unapplyFirebugLayout: function(win) {
    ...
    // Put the clear button back to the footer.
  ...
  getPanelToolbarButtons: function() {
    ...
    buttons.push({
      ...
      label: "Clear",
      tooltiptext: "Clear the request list",
    ...
    buttons.push({
      disabled: true,
      label: "Export",
      tooltiptext: "Export collecte HTTP data into HAR",
...
/**
 * The methods returns a promise that is resolved when the window
 * is loaded (win.document.readyState == 'complete').
 * If the window is already loaded the promise is resolved immediately.
 */
function waitForLoad(win) {
  ...
  if (win.document.readyState == "complete") {
    ...
    once(win, "load", () => deferred.resolve());
  }
