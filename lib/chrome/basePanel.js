...
/** @lends BasePanel */
{
  ...
  initialize: function(options) {
    // Register theme listener to receive a notification
    // when Firebug theme is applied and unapplied.
  ...
  setupSidePanels: function() {
    ...
    if (this.sidebox && (!sidePanels || !sidePanels.length)) {
      this.sidebox.hide();
      return;
    }
    ...
    for (let Panel of sidePanels) {
      ...
      let callback = this.onSidePanelLoad.bind(this, id);
      panelFrame.addEventListener("load", callback, true);
    }
  },

  onSidePanelLoad: function(id, event) {
    let xulPanel = this.sidebar.getTab(id);
    let panelFrame = xulPanel.querySelector("iframe");
