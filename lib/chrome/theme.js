...
function safeBrowserEditionTheme() {
  if (!System.isDeveloperBrowser()) {
    return;
  }
  ...
  try {
    ...
    Trace.sysout("theme.safeBrowserEditionTheme; " + state);
  } catch (err) {
    Trace.sysout("theme.safeBrowserEditionTheme; " + err, err);
  }
}
