...
Theme.registerFirebugTheme = function(options) {
  ...
  if (reason == "install" || reason == "enable" ||
    ...
    // Remember the default Developer browser edition theme for restoration
    // when Firebug is uninstalled or disabled.
    saveBrowserEditionTheme();
...
Theme.unregisterFirebugTheme = function(reason) {
  if (reason == "uninstall" || reason == "disable") {
    ...
    // browser theme.
    restoreBrowserEditionTheme();

    // Switch into the dark theme if Firebug theme is the current
    // one and the user is running developer browser.
    // See also: https://github.com/firebug/firebug.next/issues/197
    let currTheme = Services.prefs.getCharPref("devtools.theme");
    if (System.isDeveloperBrowser() && currTheme == "firebug") {
...
Theme.onPrefChanged = function(event, data) {
  ...
  toggleBrowserEditionTheme(data.newValue != "firebug");
},
...
Theme.setCurrentTheme = function(newTheme) {
...
// Developer Browser Edition Theme API

function toggleBrowserEditionTheme(enabled) {
  if (!System.isDeveloperBrowser()) {
    return;
  }

  Trace.sysout("theme.toggleBrowserEditionTheme; " + enabled);

  try {
    Services.prefs.setBoolPref("browser.devedition.theme.enabled", enabled);
  } catch (err) {
    Trace.sysout("theme.toggleBrowserEditionTheme; " + err, err);
  }
}

function saveBrowserEditionTheme() {
  if (!System.isDeveloperBrowser()) {
    return;
  }

  try {
    let state = Services.prefs.getBoolPref("browser.devedition.theme.enabled");
    prefs["developerBrowserThemeDefault"] = state;

    Trace.sysout("theme.saveBrowserEditionTheme; " + state);
  } catch (err) {
    Trace.sysout("theme.saveBrowserEditionTheme; " + err, err);
  }
}

function restoreBrowserEditionTheme() {
  if (!System.isDeveloperBrowser()) {
    return;
  }

  try {
    let state = prefs["developerBrowserThemeDefault"];
    Services.prefs.setBoolPref("browser.devedition.theme.enabled", state);

    Trace.sysout("theme.restoreBrowserEditionTheme; " + state);
  } catch (err) {
    Trace.sysout("theme.restoreBrowserEditionTheme; " + err, err);
  }
}
