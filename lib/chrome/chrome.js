...
const { TransportHooks } = require("../debug/transport-hooks.js");
...
/** @lends Chrome */
{
  ...
  initialize: function(toolbox) {
    ...
    toolbox.on("client-available", this.onCreateClient);

    target.emit("onToolboxInitialize", [this.toolbox]);
  },

  onCreateClient: function(eventId, client) {
    Trace.sysout("chrome.onCreateClient;", client);

    // Notify registered modules/services about debugger
    // client creation and destroy.
    target.emit("onToolboxCreateClient", [eventId, this.toolbox, client]);

    client.addOneTimeListener("closed", function(event) {
      target.emit("onToolboxDestroyClient", [this.toolbox, client]);
    });
  },

  onReady: function(eventId, toolbox) {
  ...
  destroy: function() {
    ...
    this.toolbox.off("client-available", this.onCreateClient);
