...
const { ToolsMenu } = require("./toolsMenu.js");
...
/** @lends Chrome */
{
  ...
  customizePanelTabs: function(apply) {
    ...
    // future? It shouldn't be set back. TESTME
    if (apply) {
      for (let tab of tabs) {
    ...
    // Append tab scrolling box to the main tab-strip (main pane tab list).
    if (apply && !this.scrollBox) {
      ...
      // Move all tabs (except of the Options tab) into the scroll box.
      Array.forEach(tabs, tab => {
        this.scrollBox.appendChild(tab)
      });
    } else if (!apply && this.scrollBox) {
      // Move all tabs back to the toolbox tabs box.
      Array.forEach(tabs, tab => {
