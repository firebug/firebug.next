 ...
 * {@Chrome} instances are created by the {@Firebug} object (singleton).
 */
const Chrome = Class(
/** @lends Chrome */
{
  ...
  onApplyTheme: function(win, oldTheme) {
    ...
    this.customizeFirebugButton(win, true);
  ...
  onUnapplyTheme: function(win, newTheme) {
    ...
    this.customizeFirebugButton(win, false);
  ...
  /**
   * Create or remove Firebug button from the main Toolbox toolbar.
   * xxxHonza: move the button into separate module + use panel.
   */
  customizeFirebugButton: function(win, apply) {
    if (apply) {
      ...
      Win.loaded(win).then(doc => {
        let button = doc.getElementById("firebug-menu");
        if (button)
          button.remove();
      })
    }
  ...
  getFirebugMenuItems: function() {
    ...
    items.push({
      nol10n: true,
      label: "About Firebug " + self.version,
