 ...
 * @param {String} option Name of the associated option. The name must
 * be specified without the Firebug namespace. Use just the last part
 * of the name (as specified in package.json). Do not use this for built
 * in preferences use {@Menu.globalOptionMenu} instead.
...
/**
 * The same as {@Menu.optionMenu}, but for global (built in) preferences.
 */
Menu.globalOptionMenu = function(label, option, tooltiptext) {
  return {
    label: label,
    type: "checkbox",
    checked: Services.prefs.getBoolPref(option),
    option: option,
    tooltiptext: tooltiptext,
    command: function() {
      let value = Services.prefs.getBoolPref(option);
      Services.prefs.setBoolPref(option, !value);

      // Fire an event for devtools.
