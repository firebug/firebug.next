...
/** @lends TabMenu */
{
  ...
  initialize: function(toolbox, panelId) {
    ...
    // These props should be part of the TabMenu object itself.
    tab.tabMenu = tabMenu;
    tab.tabMenuPopup = tabMenu.querySelector(".menuPopup");
    tab.toolbox = toolbox;

    toolbox.on("select", this.onSelectPanel);
  },
  ...
  destroy: function() {
    ...
    if (this.tab) {
      ...
      this.tab.toolbox.off("select", this.onSelectPanel);

      this.tab.tabMenu = null;
      this.tab.tabMenuPopup = null;
      this.tab.toolbox = null;
    }
  },
  ...
  onSelectPanel: function(eventId, panelId) {
    ...
    let items = this.getOptionsMenuItems(this.tab);
    if (!items || !items.length) {
      this.tab.tabMenu.setAttribute("collapsed", "true");
    }
  },
