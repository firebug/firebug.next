...
/** @lends ToolsMenu */
{
  ...
  onApplyTheme: function(win, oldTheme) {
    // xxxHonza: when 'win' is used the 'doc' passed into the callback
    // below is wrong (coming from the previous tab with active toolbox)
    // FIXME
    let view = this.toolbox.doc.defaultView;
    Win.loaded(view).then(doc => {
