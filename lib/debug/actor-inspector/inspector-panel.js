...
const panelId = "dev-panel-firebugsoftware-joehewitt-com-Actors";
...
/** @lends InspectorPanel */
{
  ...
  setup: function({debuggee}) {
    ...
    this.chrome.hooks.addListener(this);
  },

  destroy: function() {
    BasePanel.prototype.destroyapply(this, arguments);

    this.chrome.hooks.removeListener(this);
  },
  ...
  getSidePanels: function() {
    return [EditorPanel, DomSidePanel];
  },
  ...
  onReload: function() {
    ...
    let onLoad = (event) => {
      this.panelFrame.removeEventListener("load", onLoad, true);
    };

    this.panelFrame.addEventListener("load", onLoad, true);
